...
with col1:
    st.session_state.data[0, 0] = st.slider(label="Average Temperature", min_value=-8.0, max_value=30.0)

with col2:
    st.session_state.data[0, 1] = st.slider(label="Max. Temperature", min_value=-5.0, max_value=40.0)

with col3:
    st.session_state.data[0, 2] = st.slider(label="Min. Temperature", min_value=-13.0, max_value=24.5)

with col4:
    st.session_state.data[0, 3] = st.slider(label="Precipitation", min_value=0.0, max_value=52.06)

with col5:
    st.session_state.data[0, 4] = st.slider(label="Avg. Wind Speed", min_value=0.6, max_value=4.6)

with col6:
    st.session_state.data[0, 5] = st.slider(label="Max. Wind Speed", min_value=0.8, max_value=12.7)
...
with col8:
    st.session_state.data[0, 7] = st.slider(label="Max. Inst. Wind Speed ", min_value=1.8, max_value=21.5)
